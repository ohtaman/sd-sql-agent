"""
list_tables / describe_tables に knowledges を付与したツールを使う SQL エージェント。

basic_agent と instruction は同一にし、ツールのみ DatabaseToolsWithKnowledges に差し替えている。
"""
...
db_tools = DatabaseToolsWithKnowledges()

root_agent = Agent(
    model=model_name,
    name="semantic_agent",
    description="ナレッジ付き list_tables / describe_tables を使う SQL エージェント",
